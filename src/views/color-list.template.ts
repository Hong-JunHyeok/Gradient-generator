      ...
      <table class="table-auto space-y-4 min-w-full divide-y divide-gray-200">
        ...
        {{#each colorList}}
          <tr id="color-item" class="{{#ifEquals this.index ../activeColor.index}}rounded ring-4 ring-indigo-300{{/ifEquals}}">
              ...
              <td class="px-6 py-4 whitespace-nowrap"><button id="change-color" class="bg-purple-600 w-20 bg-opacity-75 rounded h-12 text-gray-100" data-index="{{this.index}}">Set Color</button></td>
          </tr>
            {{/each}}
        </tbody>
      </table>
